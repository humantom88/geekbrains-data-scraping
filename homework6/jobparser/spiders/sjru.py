...
class SjruSpider(scrapy.Spider):
    ...
    def parse(self, response:HtmlResponse):     #С этого метода все и начинается (в response - первый ответ)
        ...
        vacansy_links = response.css('a.icMQ_._6AfZ9.f-test-link-Web-razrabotchik._2JivQ _1UJAN::attr(href)').extract()
    ...
    def vacansy_parse(self, response:HtmlResponse):                         #Здесь обрабатываем информацию по вакансии
        ...
        salary_job = response.css('span._1OuF_ ZON4b::text').extract()  #Получаем зарплату в виде списка отдельных блоков
